...
- Scans game/sg/ for MP3 files (uploaded manually or downloaded)
- For each MP3, analyze beat using librosa
- Generate pattern JSON (easy/hard) to game/db/
- Create/update playlist.json based on existing MP3 files
- Also update songs.json if needed (for download purposes)
"""
...
def generate_pattern_from_beat(beat_times, bpm, is_hard=False):
    ...
    for i, t in enumerate(beat_times):
        ...
        if is_hard:
            ...
            if i % 4 == 0 and np.random.rand() < 0.3:
                duration = (60 / bpm) * 1.5
        pattern.append({"time": t, "col": col, "duration": duration})
    ...
    if is_hard and len(beat_times) > 1:
        ...
        for i in range(len(beat_times)-1):
            ...
            if (i+1) % 3 == 0:
                col = (col + 1) % 4
            double_pattern.append({"time": t_mid, "col": col, "duration": 0})
...
def save_playlist(mp3_files):
    """Buat playlist.json dari file MP3 yang ada di sg/"""
    playlist = []
...
def main():
    ...
    # Selalu update playlist.json berdasarkan file MP3 yang ada (tanpa mempengaruhi songs.json)
    save_playlist(mp3_files)
